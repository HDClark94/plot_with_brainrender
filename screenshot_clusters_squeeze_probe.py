...
def main():
    ...
    mec = scene.add_brain_region("ENTm", alpha=0.1, color=(106, 202,71), hemisphere="right", silhouette=True)
    par = scene.add_brain_region("PAR", alpha=0.1, color=(45, 160,23), hemisphere="right", silhouette=True)

    mouse_cluster_annotations_df = pd.DataFrame()
    # load mouse specific probe and cluster spatial locations
    mouse_ids = ["M20", "M21", "M25", "M26", "M27", "M28", "M29"]
    ...
    for Mouse in mouse_ids:
        ...
        clusters_df_sa = pd.read_csv(f"/Users/harryclark/Documents/brainrender/probe_data/{Mouse}_clusters.csv")
        clusters_df_ex = pd.read_csv(f'/Users/harryclark/Documents/brainrender/probe_data/extremum_channel_locations_kilosort4_0.csv')
        clusters_df_ex = clusters_df_ex[clusters_df_ex['mouse'] == mouse]
        shank_offsets_SC = shank_offsets_SC[shank_offsets_SC['mouse'] == mouse]
        clusters_df_ex = reconstruct_shank_id(clusters_df_ex, mouse)
        ...
        for sid in [0,1,2,3]:
            print(f"Point (0,0) on the probe maps to {brain_coord_CCF} in the CCF format.")
            brain_coord_SC, brain_coord_CCF = brain_coord_from_xy(0,0*0.943, adjusted_probe_locs_SC, shank_id=sid)
            scene.add(Points(np.reshape(brain_coord_CCF, (1,3)), radius=50, colors="blue"))
            brain_coord_SC, brain_coord_CCF = brain_coord_from_xy(0,500*0.943, adjusted_probe_locs_SC, shank_id=sid)
            scene.add(Points(np.reshape(brain_coord_CCF, (1,3)), radius=50, colors="blue"))
            brain_coord_SC, brain_coord_CCF = brain_coord_from_xy(0,1000*0.943, adjusted_probe_locs_SC, shank_id=sid)
            scene.add(Points(np.reshape(brain_coord_CCF, (1,3)), radius=50, colors="blue"))
            brain_coord_SC, brain_coord_CCF = brain_coord_from_xy(0,1500*0.943, adjusted_probe_locs_SC, shank_id=sid)
            scene.add(Points(np.reshape(brain_coord_CCF, (1,3)), radius=50, colors="blue"))
            brain_coord_SC, brain_coord_CCF = brain_coord_from_xy(0,2000*0.943, adjusted_probe_locs_SC, shank_id=sid)
            scene.add(Points(np.reshape(brain_coord_CCF, (1,3)), radius=50, colors="blue"))
            brain_coord_SC, brain_coord_CCF = brain_coord_from_xy(0,2500*0.943, adjusted_probe_locs_SC, shank_id=sid)
            scene.add(Points(np.reshape(brain_coord_CCF, (1,3)), radius=50, colors="blue"))
        '''
        ...
        cluster_curated = []
        for index, cluster in clusters_df_ex.iterrows():
            cluster_id = int(cluster['cluster_id'])
            shank_id = int(cluster['shank_id'])
            x_pos = cluster['unit_location_x']
            y_pos = cluster['unit_location_y']
            day = cluster['day']
            cluster_sa= clusters_df_sa[(clusters_df_sa['Day'] == f'D{str(int(day))}') & 
                                       (clusters_df_sa['cluster_id'] ==cluster_id)]
            firing_rate = cluster_sa['firing_rate'].iloc[0]

            # give it a curated label if it passes some basic thresholding
            if len(cluster_sa[(cluster_sa["snr"] > 1) & 
                              (cluster_sa["firing_rate"] > 0.5) & 
                              (cluster_sa["rp_contamination"] < 0.9)]) ==1:
                curated=True
            else:
                curated=False
            ...
            # squeeze y position of the spike by a factor of 0.943 as described in the blog post
            y_pos *= 0.943
            ...
            cluster_firing_rates.append(firing_rate)
            cluster_curated.append(curated)
        ...
        cluster_curated = np.array(cluster_curated)
        cluster_annotation_colors = get_annotation_colors(cluster_annotations)

        clusters_df_ex['Mouse'] = Mouse
        clusters_df_ex['mouse'] = mouse
        clusters_df_ex['coord_SCs_z'] = cluster_coord_SCs_z
        clusters_df_ex['coord_SCs_y'] = cluster_coord_SCs_y
        clusters_df_ex['coord_SCs_x'] = cluster_coord_SCs_x
        clusters_df_ex['coord_CCFs_z'] = cluster_coord_CCFs_z
        clusters_df_ex['coord_CCFs_y'] = cluster_coord_CCFs_y
        clusters_df_ex['coord_CCFs_x'] = cluster_coord_CCFs_x
        clusters_df_ex['firing_rate'] = cluster_firing_rates
        clusters_df_ex['curated'] = cluster_curated
        clusters_df_ex['brain_region'] = cluster_annotations
        mouse_cluster_annotations_df = pd.concat([mouse_cluster_annotations_df, clusters_df_ex], ignore_index=True)

        cluster_coord_CCFs = np.array(cluster_coord_CCFs)[cluster_curated] # only plot curated cells.
        cluster_annotation_colors = np.array(cluster_annotation_colors)[cluster_curated] # only plot curated cells.
        scene.add(Points(np.reshape(cluster_coord_CCFs, (len(cluster_coord_CCFs),3)), radius=20, colors='black', alpha=0.1))

    annotations = np.array(mouse_cluster_annotations_df[mouse_cluster_annotations_df['curated']==True]['brain_region']).tolist()
    ...
    for substring in ['ENT','VIS','PRE','HPF','SUB','PAR','SIM','arb','PFL', 'root']:
    ...
    camera_configs = {
        "top": {
            "pos": [0, -20000, 0],      # Anterior (front)
            "viewup": [0, 0, 1],
            "clipping_range": [1, 100000],
            "focal_point": [0, 0, 0],
        },
        "back": {
            "pos": [20000, 0, 0],       # Right
    ...
    name ='sagittal'
    if name != 'sagittal':
        scene.screenshot(name=f"/Users/harryclark/Desktop/Nature2026 Figs/{name}.png", scale=20, camera=camera_configs[name], resetcam=True, zoom=1.2)
    else: 
        scene.screenshot(name=f"/Users/harryclark/Desktop/Nature2026 Figs/{name}.png", scale=20, camera=name, resetcam=True, zoom=1.2)
