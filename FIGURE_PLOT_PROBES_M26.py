...
def main():
    ...
    mec = scene.add_brain_region("ENTm", alpha=0.25, color="#13A4D5", hemisphere="right", silhouette=True)
    par = scene.add_brain_region("PAR", alpha=0.25, color="#003590", hemisphere="right", silhouette=True)
    ...
    for Mouse in mouse_ids:
        ...
        for i in range(len(adjusted_probe_locs_CCF)):
            if i == 2:
                adjusted_probe_locs_CCF[i, 2, 0] += 75
                adjusted_probe_locs_CCF[i, 2, 1] += 75

            actor = Cylinder2(adjusted_probe_locs_CCF[i, :, 0], 
                            adjusted_probe_locs_CCF[i, :, 1], scene.root, color='black', radius=30)
